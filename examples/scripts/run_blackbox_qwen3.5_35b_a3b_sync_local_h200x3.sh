...
sleep 3
pkill -9 ray
pkill -9 -f "python3 -m dressage.proxy.server" || true
pkill -9 -f "python3 train.py" || true
pkill -9 opencode
pkill -9 bwrap
sleep 2
...
# Reserve GPU 3 by default. Override this env var explicitly if a different
# GPU set is intended.
...
RAY_NUM_GPUS_PER_NODE=${RAY_NUM_GPUS_PER_NODE:-3}
CP_SIZE=${CP_SIZE:-2}
...
dressage_compute_context_window 12288 "${CP_SIZE}"   # MTP 训练加层+额外词表投影,降 token 预算给 GPU0(head rank)留余量
...
CKPT_ARGS=(
  ...
  #  --ref-load "${BASE_FOLDER}/Qwen3.5-35B-A3B_torch_dist_mtp"
   --ref-load "${BASE_FOLDER}/Qwen3.5-35B-A3B"
...
#    --wandb-group qwen3.5-35B-A3B-mtp-eagle-fp8
#    --no-inference-wandb-logging
# )
...
if [[ "${SGLANG_ENABLE_LAYERWISE_NVTX_MARKER:-0}" == "1" ]]; then
  SGLANG_ARGS+=(--sglang-enable-layerwise-nvtx-marker)
fi
...
export DRESSAGE_LOCAL_BWRAP_ACQUIRE_CONCURRENCY="${DRESSAGE_LOCAL_BWRAP_ACQUIRE_CONCURRENCY:-32}"
export DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_PRECHECK="${DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_PRECHECK:-1}"
export DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_RECYCLE_ATTEMPTS="${DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_RECYCLE_ATTEMPTS:-1}"
export DRESSAGE_BLACKBOX_REGISTER_CONCURRENCY="${DRESSAGE_BLACKBOX_REGISTER_CONCURRENCY:-32}"
export DRESSAGE_BLACKBOX_REGISTER_RECYCLE_ATTEMPTS="${DRESSAGE_BLACKBOX_REGISTER_RECYCLE_ATTEMPTS:-1}"
...
RUNTIME_ENV_JSON=$(cat <<EOF_JSON
{
  "env_vars": {
    ...
    "DRESSAGE_PROFILE": "${DRESSAGE_PROFILE:-0}",
    ...
    "DRESSAGE_BLACKBOX_NODE_ACQUIRE_TIMEOUT_SEC": "${DRESSAGE_BLACKBOX_NODE_ACQUIRE_TIMEOUT_SEC}",
    "DRESSAGE_BLACKBOX_BACKGROUND_RELEASE_POLL_SEC": "${DRESSAGE_BLACKBOX_BACKGROUND_RELEASE_POLL_SEC}",
    "DRESSAGE_LOCAL_BWRAP_ACQUIRE_CONCURRENCY": "${DRESSAGE_LOCAL_BWRAP_ACQUIRE_CONCURRENCY}",
    "DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_PRECHECK": "${DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_PRECHECK}",
    "DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_RECYCLE_ATTEMPTS": "${DRESSAGE_LOCAL_BWRAP_ACQUIRE_HEALTH_RECYCLE_ATTEMPTS}",
    "DRESSAGE_BLACKBOX_REGISTER_CONCURRENCY": "${DRESSAGE_BLACKBOX_REGISTER_CONCURRENCY}",
    "DRESSAGE_BLACKBOX_REGISTER_RECYCLE_ATTEMPTS": "${DRESSAGE_BLACKBOX_REGISTER_RECYCLE_ATTEMPTS}",
